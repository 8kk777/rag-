package com.yizhaoqi.rag.controller;

import com.yizhaoqi.rag.config.KafkaConfig;
import com.yizhaoqi.rag.exception.CustomException;
import com.yizhaoqi.rag.model.FileProcessingTask;
import com.yizhaoqi.rag.model.FileUpload;
import com.yizhaoqi.rag.model.OrganizationTag;
import com.yizhaoqi.rag.repository.FileUploadRepository;
import com.yizhaoqi.rag.service.FileTypeValidationService;
import com.yizhaoqi.rag.service.ParseService;
import com.yizhaoqi.rag.service.UploadService;
import com.yizhaoqi.rag.service.UserService;
import com.yizhaoqi.rag.utils.LogUtils;
...
public class UploadController {
    ...
    public ResponseEntity<Map<String, Object>> uploadDirect(
        ...
        LogUtils.PerformanceMonitor monitor = LogUtils.startPerformanceMonitor("UPLOAD_DIRECT");
        try {
            FileTypeValidationService.FileTypeValidationResult validationResult =
                    fileTypeValidationService.validateFileType(fileName);
            if (!validationResult.isValid()) {
                monitor.end("direct upload failed: invalid file type");
                Map<String, Object> errorResponse = new HashMap<>();
                errorResponse.put("code", HttpStatus.BAD_REQUEST.value());
                errorResponse.put("message", validationResult.getMessage());
                errorResponse.put("fileType", validationResult.getFileType());
            ...
            if (!userService.isAdminUser(userId)) {
                ...
                boolean exceedsLimit = uploadMaxSizeBytes != null
                        && uploadMaxSizeBytes > 0
                        && totalSize > uploadMaxSizeBytes;
                if (exceedsLimit) {
                    monitor.end("direct upload failed: file size limit exceeded");
                    Map<String, Object> errorResponse = new HashMap<>();
                    errorResponse.put("code", HttpStatus.PAYLOAD_TOO_LARGE.value());
                    errorResponse.put("message", "Current organization limits upload size to " + formatSize(uploadMaxSizeBytes)
                            + "; current file size is " + formatSize(totalSize));
                    errorResponse.put("limitBytes", uploadMaxSizeBytes);
                    errorResponse.put("fileSizeBytes", totalSize);
                    errorResponse.put("orgTag", orgTag);
                    return ResponseEntity.status(HttpStatus.PAYLOAD_TOO_LARGE).body(errorResponse);
            ...
            LogUtils.logFileOperation(userId, "UPLOAD_DIRECT", fileName, fileMd5, "PROCESSING");
            String objectUrl = uploadService.uploadDirect(fileMd5, totalSize, fileName, file, orgTag, isPublic, userId);
            LogUtils.logFileOperation(userId, "UPLOAD_DIRECT", fileName, fileMd5, "SUCCESS");

            FileUpload fileUpload = fileUploadRepository.findFirstByFileMd5AndUserIdOrderByCreatedAtDesc(fileMd5, userId)
                    .orElseThrow(() -> new RuntimeException("File upload record does not exist: " + fileMd5));

            ParseService.EmbeddingEstimate embeddingEstimate = null;
            try (io.minio.GetObjectResponse mergedFileStream = uploadService.getMergedFileStream(fileMd5)) {
                ...
                LogUtils.logBusinessError(
                        "UPLOAD_DIRECT",
                        userId,
                        "Embedding estimate failed: fileMd5=%s, fileName=%s",
                        estimateException,
                        fileMd5,
                        fileName
                );
            ...
            Map<String, Object> response = new HashMap<>();
            response.put("code", 200);
            response.put("message", "Direct upload succeeded; processing task has been sent to Kafka");
            response.put("data", data);

            LogUtils.logUserOperation(userId, "UPLOAD_DIRECT", fileMd5, "SUCCESS");
            monitor.end("direct upload succeeded");
            return ResponseEntity.ok(response);
        } catch (CustomException e) {
            monitor.end("direct upload failed: " + e.getMessage());
            Map<String, Object> errorResponse = new HashMap<>();
            errorResponse.put("code", e.getStatus().value());
            errorResponse.put("message", e.getMessage());
            return ResponseEntity.status(e.getStatus()).body(errorResponse);
        } catch (Exception e) {
            LogUtils.logBusinessError("UPLOAD_DIRECT", userId, "Direct upload failed: fileMd5=%s, fileName=%s", e, fileMd5, fileName);
            monitor.end("direct upload failed: " + e.getMessage());
            Map<String, Object> errorResponse = new HashMap<>();
            errorResponse.put("code", HttpStatus.INTERNAL_SERVER_ERROR.value());
            errorResponse.put("message", "Direct upload failed: " + e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(errorResponse);
        }
    }
